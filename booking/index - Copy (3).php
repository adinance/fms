...
<head>
  ...
  <style>
    ...
    /* Position for Table Toggle FAB */
    #toggleTable {
        background-color: #007bff; /* สีน้ำเงินสำหรับการสลับตาราง */
        font-size: 20px;
        bottom: 170px; /* วางไว้เหนือปุ่ม Theme Toggle */
    }
...
<nav class="navbar navbar-expand-lg navbar-light bg-light shadow-sm py-0 border-bottom">
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                ...
                <ul class="navbar-nav">
                    <li class="nav-item">
                        <a class="nav-link" href="#"><i class="bi bi-bar-chart-line-fill"></i> Reporting</a>
                    </li>
                    <li class="nav-item">
                    ...
                    <li class="nav-item dropdown ms-2">
                        <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                            <i class="bi bi-person-circle"></i> <span class="d-lg-none d-xl-inline">นาย A</span>
                        </a>
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  initCalendars();
  initTable();
  // 🔥 NEW: โหลดสถานะ Checkbox ก่อนตั้งค่า Handler
  loadRoomFilterState(); 
  initEventHandlers();

  // ----------------------------------------------------------------------
  // ✅ แก้ไขแล้ว: ฟังก์ชันสำหรับบันทึก/โหลดสถานะ Checkbox 
  // ----------------------------------------------------------------------

  function saveRoomFilterState() {
    // 1. บันทึกสถานะ "เลือกทั้งหมด"
    const checkAll = $("#checkAllRooms").is(":checked");
    localStorage.setItem("checkAllRoomsState", checkAll);

    // 2. บันทึกสถานะของแต่ละห้อง
    const roomStates = {};
  ...
  function loadRoomFilterState() {
    // 1. โหลดสถานะ "เลือกทั้งหมด"
    const savedCheckAll = localStorage.getItem("checkAllRoomsState");
    // ✅ แก้ไข: ถ้า savedCheckAll ไม่ใช่ null (คือมีการบันทึกค่าไว้แล้ว) ให้ใช้ค่านั้น
    if (savedCheckAll !== null) {
        $("#checkAllRooms").prop("checked", savedCheckAll === "true");
    } else {
        // หากไม่เคยบันทึก (โหลดครั้งแรก) ให้ใช้ค่าเริ่มต้นจาก HTML (checked)
        $("#checkAllRooms").prop("checked", true); 
    }

    // 2. โหลดสถานะของแต่ละห้อง
    ...
    // สำคัญ: เรียกใช้ Filter ทันทีหลังโหลดสถานะเพื่อปรับปฏิทินและตาราง
    applyRoomFilter(true); 
  }

  // ----------------------------------------------------------------------
  // (ฟังก์ชันจัดการ Theme และ Table Visibility)
  // ----------------------------------------------------------------------
  ...
  function initEventHandlers() {
    // MODIFIED: เพิ่มการเรียก saveRoomFilterState
